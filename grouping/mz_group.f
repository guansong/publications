...
! ... This version is based on the 'ONTHREADS' construct from the SUBTEAM proposal
! ... by B. Chapman et al.
...
! ... Initialize the group, make it more like a lock
      CALL OMP_INIT_PROCS(GROUP)
! ... I'm assuming the system provided a flat structure.
      GROUP=OMP_GET_PROCS()
! ... This line is just a safe checking 
      IF (OMP_PROCS_GET_NUM_MEMBERS(GROUP).GT.max_threads) STOP

! ... Assume this can work on an array, otherwise use a DO loop
      CALL OMP_INIT_PROCS(GROUPARRAY)

! ... This line will put them into GROUPARRAY
...
! ... If I understand the code correctly, your group_subid
! ... includes each threads's group number
! ... I changed it to a different format as each group's start-end tid
...
!      subteam = omp_create_subteam(group_subid, mgid)
      ...
      IF (OMP_PROCS_SET_MEMBERS(SUBTEAM, GROUPARRAY(group_start(mgid):group_end(mgid))).NE.0) STOP
